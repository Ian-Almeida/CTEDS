using Aula2_Prog_Avanc_DB.Interfaces;
...
namespace Aula2_Prog_Avanc_DB.Repositories
{
    internal class BaseRepository<T> : IBaseRepository<T> where T : new()
    {
        ...
        public Fields GetClassFields(T payload)
        {
            ...
            fieldsNames = typeof(T).GetFields()
            ...
            {
                FieldInfo info = payload.GetType().GetField(field, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
                if (info != null)
                {
                    classFields.Add(new Tuple<string, FieldInfo>(field, info));
                }
            }
