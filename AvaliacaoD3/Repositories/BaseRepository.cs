...
namespace AvaliacaoD3.Repositories
{
    ...
    {
        protected readonly string CONNECTION_STRING = "Server=localhost;Database=AvaliacaoD3;Trusted_Connection=True;";

        protected string tableName = string.Empty;

        protected string idField = string.Empty;
            ...
            using (SqlConnection conn = new SqlConnection(CONNECTION_STRING))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    foreach (var field in fields.classFields)
                    {
                        ...
                        {
                            cmd.Parameters.AddWithValue("@" + field.Item1, field.Item2.GetValue(payload).ToString());
                            continue;
                        }
